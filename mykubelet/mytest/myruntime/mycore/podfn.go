...
func (this *PodFn) SyncPodFn(ctx context.Context, updateType kubetypes.SyncPodType, pod *v1.Pod, mirrorPod *v1.Pod, podStatus *kubecontainer.PodStatus) (bool, error) {
	...
	if updateType == kubetypes.SyncPodCreate {
		if pod.Name == "ngx-mylain" {
			...
			podStatus := this.generateAPIPodStatus(pod, podStatus)
			pod.Status = podStatus

			// 置空，否则会因为版本落后（原因未知）导致报错： Operation cannot be fulfilled on pods "ngx-mylain": the object has been modified; please apply your changes to the latest version and try again
			pod.ResourceVersion = ""

			_, err := this.KubeClient.CoreV1().Pods(pod.Namespace).UpdateStatus(ctx, pod, metav1.UpdateOptions{})
			if err != nil {
				log.Println(err)
			}
		}
...
func (this *PodFn) generateAPIPodStatus(pod *v1.Pod, podStatus *kubecontainer.PodStatus) v1.PodStatus {
	...
	oldPodStatus, found := this.StatusManager.GetPodStatus(pod.UID)
	if !found {
		oldPodStatus = pod.Status
	}
	s := this.convertStatusToAPIStatus(pod, podStatus, oldPodStatus)
...
func (this *PodFn) convertStatusToAPIStatus(pod *v1.Pod, podStatus *kubecontainer.PodStatus, oldPodStatus v1.PodStatus) *v1.PodStatus {
...
func (this *PodFn) convertToAPIContainerStatuses(pod *v1.Pod, podStatus *kubecontainer.PodStatus, previousStatus []v1.ContainerStatus, containers []v1.Container, hasInitContainers, isInitContainer bool) []v1.ContainerStatus {
	...
	for _, container := range containers {
		...
		status := statuses[container.Name]
		reason, ok := this.ReasonCache.Get(pod.UID, container.Name)
		if !ok {
...
type MyPodDeletionSafetyProvider struct{}

func (m MyPodDeletionSafetyProvider) PodResourcesAreReclaimed(pod *v1.Pod, status v1.PodStatus) bool {
	return true
}

func (m MyPodDeletionSafetyProvider) PodCouldHaveRunningContainers(pod *v1.Pod) bool {
	return true
}
