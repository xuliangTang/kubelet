...
func (m *MyKubelet) generateAPIPodStatus(pod *v1.Pod, podStatus *kubecontainer.PodStatus) v1.PodStatus {
	...
	oldPodStatus, found := m.statusManager.GetPodStatus(pod.UID)
	if !found {
		oldPodStatus = pod.Status
	}
	s := m.convertStatusToAPIStatus(pod, podStatus, oldPodStatus)
	...
	m.probeManager.UpdatePodStatus(pod.UID, s)
...
func (m *MyKubelet) convertStatusToAPIStatus(pod *v1.Pod, podStatus *kubecontainer.PodStatus, oldPodStatus v1.PodStatus) *v1.PodStatus {
	...
	apiPodStatus.ContainerStatuses = m.convertToAPIContainerStatuses(
		pod, podStatus,
	...
	apiPodStatus.InitContainerStatuses = m.convertToAPIContainerStatuses(
		pod, podStatus,
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.EphemeralContainers) {
		...
		apiPodStatus.EphemeralContainerStatuses = m.convertToAPIContainerStatuses(
			pod, podStatus,
...
func (m *MyKubelet) convertToAPIContainerStatuses(pod *v1.Pod, podStatus *kubecontainer.PodStatus, previousStatus []v1.ContainerStatus, containers []v1.Container, hasInitContainers, isInitContainer bool) []v1.ContainerStatus {
	...
	for _, container := range containers {
		...
		status := statuses[container.Name]
		reason, ok := m.reasonCache.Get(pod.UID, container.Name)
		if !ok {
